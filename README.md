...
- [x] **PWA/모바일 앱**: 모바일 하이브리드 앱 패키징(Capacitor) 뼈대 구축 및 iOS 설정(ATS, 터치 버그 수정) 완료
- [x] **CI/CD 자동화**: 깃허브 액션(GitHub Actions)을 활용한 자동 AWS 서버 배포 파이프라인 구축 완료
...
- [x] **사업자/어드민 대시보드 실데이터 연동**: 가짜 데이터를 모두 제거하고 실제 DB 기반 통계 및 목록 API 연동 완료
- [x] **앱/프로덕션 배포 시 환경 설정**: CORS 고도화 및 `.env` 파일의 `VITE_API_BASE_URL` 실서버 반영, 하이브리드 절대경로 변환 완료
