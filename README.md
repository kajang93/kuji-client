...
- **3D 물리 엔진 기반 뜯기(Reveal)**: Framer Motion의 Physics 엔진(Elastic Tension, Snap)을 활용하여 진짜 종이를 찢는 듯한 저항감과 스냅백, 햅틱 진동(Vibration API)을 완벽 구현.
- **프리미엄 연출**: 당첨 증서 등장 시 `react-tilt`를 활용한 3D 입체 카드 효과와 `canvas-confetti` 파티클 액션 적용.
...
### 🌟 2026 트렌디 킬러 기능 (향후 로드맵)
- **숏폼(Reels/Shorts) 비디오 자동 렌더링**: 당첨 뜯기 애니메이션을 3~5초 세로형 영상으로 자동 생성하여 SNS 바이럴 유도
- **WebAR (증강현실) 피규어 배치**: 별도 앱 설치 없이 스마트폰 카메라로 내 책상 위에 A상/B상 피규어를 미리 띄워보는 3D AR 기능
- **미니게임 게이미피케이션(Gamification)**: 출석체크 대신 '최애 밥 주기' 미니게임을 도입하여 리텐션 극대화 (무료 쿠지 보상)
- **Provably Fair (공정성 증명)**: 추첨 시드 해시값을 블록체인/스마트컨트랙트 원리로 투명하게 공개하여 조작 의혹 원천 차단

### 보완 작업 (Pending Tasks) 📝
- **상세 옵션 팝업(PrizeSelection) 실데이터 연동**: 기존 단일 상품의 `options` 배열 의존 구조에서, 동일 등급(rank)의 다중 개별 상품들을 가져오도록 로직 완전 교체(실제 데이터 연동 완료) ✅
- **UI 텍스트 및 레이아웃 개선**: 고객용 시리즈 화면 영문 텍스트 한글화 적용 및 당첨 팝업창 레이아웃(스크롤, 버튼 너비 등) 개선 완료 ✅
- **고객센터 채널 연동**: 설정 화면(`Settings.tsx`)의 목데이터 이메일 텍스트를 카카오톡 1:1 채널 문의 버튼(`pf.kakao.com`)으로 전면 교체 완료 ✅
- **사업자 상품 등록 UI 고도화**: 단품(Single) / 다종(Multi) 구분 기능과 동적 상(Rank) 추가 시스템 구현 완료 ✅ 그리고 목업(MOCK) 상품 옵션 데이터를 전면 제거하고 API를 통해 실제 상품의 등급별 남은 수량과 세부 정보 실데이터를 맵핑해 등록/수정에 연동 완료 ✅
...
- **프론트엔드 이미지 최적화**: 10MB 용량 제한 예외 처리 및 `browser-image-compression`을 활용한 모바일 1MB 자동 압축 기능 적용 완료 ✅
- **에러 핸들링 고도화**: API 응답 중 튀어나오는 원시 JSON 에러 데이터를 파싱하여, 순수 텍스트 메시지만 팝업창(Toast)에 노출되도록 헬퍼 전면 적용 완료 ✅
...
- **모바일 기기 디버깅 최적화**: 외부 기기(아이폰 등)에서 로컬 네트워크 접속 시 발생하는 Vite HMR 웹소켓 Connection Refused 오류 해결(server.host 설정) 완료 ✅
- **관리자/사업자 편의성**: 상품 등록 시 AI 백그라운드 제거 및 설명 자동 생성 로직(processAiImage) 프론트엔드 연동 완료 ✅
