...
### 보완 작업 (Pending Tasks) 📝
- **사업자 상품 등록 및 옵션 연동 고도화**: 단품(Single) / 다종(Multi) 구분 및 동적 상(Rank) 추가 기능 구현, 그리고 목업(MOCK) 상품 옵션 데이터를 전면 제거하고 API를 통해 실제 상품의 등급별 남은 수량과 세부 정보 실데이터를 맵핑해 등록/수정에 연동 완료 ✅
